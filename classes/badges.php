...
//            echo 'Anzahl ausgestellter Zertifikate (' . get_string('only_for_trainers', 'block_oc_mooc_nav') . '): ' . count($cert_issues);
        ...
        if ($issue_count > 0) {
            echo 'Anzahl ausgestellter Zertifikate (' . get_string('only_for_trainers', 'block_oc_mooc_nav') . '): ' . $issue_count;
        }

        echo \html_writer::tag('h2', \html_writer::tag('div', get_string('certificate', 'block_oc_mooc_nav'), array('class' => 'oc_badges_text')));
// echo html_writer::tag('div', html_writer::tag('div', get_string('cert_addtext', 'block_oc_mooc_nav'), array('class' => 'oc_badges_text')));
        if ($certificate_m && ($coursecert_cm = $DB->get_record('course_modules', array('module' => $certificate_m->id, 'course' => $courseid, 'visible' => 1)))) {
            ...
            if ($course_cert) {
                echo \html_writer::tag('div', get_string('cert_descr_general', 'block_oc_mooc_nav'));
